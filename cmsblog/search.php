...
<body>
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-md-8">
                ...
                try {
                    ...
                    $posts = Post::getPosts('published', 'DESC', $start, $count, $catID, $authorID, $search);
                ...
                <p class='lead'>by <a href='author_posts.php?author=$objUser->userID'>$objUser->userName</a></p>
                <p><span class='glyphicon glyphicon-time'></span> Posted on $p->postDate</p>
                <hr>
                <a href='post.php?post_id=$p->postID'><img class='img-responsive' src='images/posts/$p->postName/$p->postImage' alt='$p->postImage'>
                </a>
                <hr>
        ...
        <ul class="pager">
            <?php
            try {
                $pNums = Post::pageination(ITEM_PER_PAGE, $catID, $authorID, $search);

                foreach ($pNums as $pNo => $start) {
                    echo "<li><a href='" . BASE_URL . "search.php?start=$start&search=$search'>$pNo</a> </li>";
                }
            } catch (Exception $ex) {
                echo ($ex->getMessage());
            }
